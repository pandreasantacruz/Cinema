...
const getMovieByIdService = (id) => {
  const movieToBeFound = movies.find((movie) => movie.id === Number(id));
  if (movieToBeFound) return [movieToBeFound];
  return [];
};
const postMovieService = (movie) => {
  console.log("Datos recibidos en el servicio:", movie);
  movies.push(movie);
  console.log("Estado del array movies:", movies);
  return movie;
};
